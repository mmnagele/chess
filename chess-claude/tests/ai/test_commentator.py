...
def test_commentator_provides_commentary() -> None:
    ...
    assert "Current mode: COMMENTARY" in provider.requests[0].system_prompt
...
def test_commentator_chat() -> None:
    ...
    assert "Current mode: CHAT" in provider.requests[-1].system_prompt
    assert "MODE=CHAT" in provider.requests[-1].user_prompt
    assert "Commentator type: Tournament Commentator" in provider.requests[-1].user_prompt
